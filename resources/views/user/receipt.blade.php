	...
	<div id="page-wrap" >
		...
		<div id="customer">
			...
			@foreach($payment as $pay)
                ...
                <tr>
                    ...
                    <td><div class="due">{{ number_format($data['total_due'], 2) }}</div></td>
                </tr>
			@endforeach
            </table>
		...
		@foreach($payment as $pay)
		  ...
		  <tr class="item-row">
		      ...
		      <td><h4 class="cost"><span>N</span>{{ number_format($data['current_billing'], 2) }}</h4></td>
		      <td><h4 class="qty">1</h4></td>
		      <td><span class="price"><span>N</span>{{ number_format($data['current_billing'], 2) }}</span></td>
		  </tr>
		  
		  <tr>
		      <td colspan="2" class="blank"> </td>
		      <td colspan="2" class="total-line">Subtotal</td>
		      <td class="total-value"><div id="subtotal"><span>N</span>{{ $pay->amount }}</div></td>
		  </tr>
		  <tr>

		      <td colspan="2" class="blank"> </td>
		      <td colspan="2" class="total-line">Total</td>
		      <td class="total-value"><div id="total"><span>N</span>{{ $pay->amount }}</div></td>
		  </tr>
		  <tr>
		      <td colspan="2" class="blank"> </td>
		      <td colspan="2" class="total-line">Amount Paid</td>

		      <td class="total-value"><h4 id="paid"><span>N</span>{{ $pay->amount }}</h4></td>
		  </tr>
		  <tr>
		      <td colspan="2" class="blank"> </td>
		      <td colspan="2" class="total-line balance">Balance Due</td>
		      <td class="total-value balance"><div class="due"><span>N</span>{{ number_format($data['total_due'], 2) }}</div></td>
		  </tr>
		@endforeach
		</table>
